...
from dataset import generate_phase_one_dataloader
...
def perform_training(args, cfg, peft_config):
    ...
    if args.phase_one:
        ...
        mllm = PhiClipMLLM.from_pretrained(
            checkpoint_path=cfg["checkpoint"]["load_dir"],
            dtype=torch.bfloat16 if cfg["dtype"] == "torch.bfloat16" else torch.float32,
        )
    ...
    training_args = TrainingArguments(
        ...
        logging_steps=cfg["training_args"].get(
            "logging_steps", 10
    ...
    optimizer = AdamW(
        mllm.parameters(), lr=cfg["training_args"]["learning_rate"], weight_decay=0.001
    )
...
def main():
    ...
    parser.add_argument(
        "--phase_one", action="store_true", help="Run phase one training."
    )
    parser.add_argument(
    ...
    if "peft_adapter" in cfg and cfg["peft_adapter"]:
        peft_config_adapter = LoraConfig(
            ...
            target_modules=cfg["peft_adapter"].get(
    ...
    perform_training(
        args, cfg, [peft_config_text, peft_config_vision, peft_config_adapter]
    )
