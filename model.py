...
import os
from transformers import CLIPVisionModelWithProjection, AutoTokenizer, AutoModelForCausalLM, AutoProcessor, CLIPVisionModel
    ...
    def _initialize_weights(self):
        ...
        for module in self.projector:
            if isinstance(module, nn.Linear):
                nn.init.kaiming_uniform_(module.weight, nonlinearity='relu')  # He initialization
                if module.bias is not None:
                    nn.init.zeros_(module.bias)
                    

# class MultimodalProjector(nn.Module):
#     def __init__(self, image_embedding_dim, text_embedding_dim, hidden_dim=None, activation=nn.ReLU, normalization=True, dropout=0.1):
#         """
#         A projection layer to align image embeddings with the text embedding space.
...
#             text_embedding_dim (int): Dimensionality of text embeddings in the text model.
#             hidden_dim (int, optional): Optional intermediate hidden dimension for the projector. Defaults to None.
#             activation (callable, optional): Activation function to apply after the projection. Defaults to nn.GELU.
#             normalization (bool, optional): Whether to apply LayerNorm normalization to the output. Defaults to True.
...
#         if hidden_dim is not None:
#             # If a hidden dimension is specified, use a multi-layer projection with bottleneck
#             layers.append(nn.Linear(image_embedding_dim, hidden_dim))
#             if normalization:
...
#             layers.append(nn.Linear(hidden_dim, hidden_dim // 2))  # Bottleneck layer
#             if normalization:
...
#             layers.append(nn.Linear(hidden_dim // 2, text_embedding_dim))
#         else:
#             # If no hidden dimension, use a single linear layer
#             layers.append(nn.Linear(image_embedding_dim, text_embedding_dim))

#         if normalization:
#             layers.append(nn.LayerNorm(text_embedding_dim))
...
class PhiClipMLLM(nn.Module):
    ...
    def __init__(
        ...
        self.vision_model = CLIPVisionModel.from_pretrained(vision_model, torch_dtype=dtype).to(self.device)
        self.vision_processor = AutoProcessor.from_pretrained(vision_model)
        self.text_model = AutoModelForCausalLM.from_pretrained(language_model, torch_dtype=dtype, low_cpu_mem_usage=True).to(self.device)
    ...
    def forward(self, batch):
        ...
        combined_embedding = self.combine_text_image(text_embeddings, adapted_image_embeddings, img_token_mask)
        ...
        for batch_idx, mask in enumerate(img_token_mask):
            ...
            image_mask = torch.ones(adapted_image_embeddings.size(1), device=attention_mask.device)
        ...
        # Compute logits
        outputs = self.text_model(inputs_embeds=combined_embedding, attention_mask=expanded_attention_mask, position_ids = position_ids)
        ...
        if labels is not None:
            labels = [
                text.replace("<|placeholder1|>", "<|placeholder1|>" * 257) for text in labels
            ]
            # Shift labels to align with predicted outputs for loss computation
            label_tokens = self.encode_text(labels, token_only = True, instructions = self.instuction_training)
    ...
    def __call__(self, input_texts, input_images, labels=None):
        return self.forward({"input_texts": input_texts, "input_images": input_images, "labels": labels})
    ...
    def encode_text(self, input_text, token_only = False, instructions = False):
        ...
        tokenized = self.tokenizer(input_text, return_tensors="pt", padding=True,
                                   max_length=512, return_attention_mask=True, truncation=True)
        input_ids = tokenized["input_ids"]
        attention_mask = tokenized["attention_mask"]
        if token_only:
            if instructions:
                instruction_placeholder_positions = (input_ids == self.tokenizer.convert_tokens_to_ids(_PLACEHOLDER_INSTRUCT)).nonzero(as_tuple=False)
        ...
        text_embeddings = self.text_model.get_input_embeddings()(input_ids.to(self.device))
    ...
    def save_checkpoint(self, save_path: Union[str, Path]):
        ...
        checkpoint = {
            ...
            "vision_adapter_config": {
                ...
                "hidden_dim": self.vision_adapter.projector[0].out_features if len(self.vision_adapter.projector) > 1 else None,
                "activation": type(self.vision_adapter.projector[1]) if len(self.vision_adapter.projector) > 2 else None,
    ...
    def load_checkpoint(
        ...
        try:
            ...
            # Load model states
            self.vision_model.load_state_dict(checkpoint["vision_model_state"], strict=strict)
            self.vision_adapter.load_state_dict(checkpoint["vision_adapter_state"], strict=strict)
            self.text_model.load_state_dict(checkpoint["text_model_state"], strict=strict)
            ...
            if current_processor_type != checkpoint_processor_type:
                print(f"Warning: Current processor type ({current_processor_type}) "
    ...
    def from_pretrained(
        ...
        checkpoint["vision_model_state"] = {k.replace('base_model.model.', ''): v 
                    for k, v in checkpoint["vision_model_state"].items()}
        checkpoint["text_model_state"] = {k.replace('base_model.model.', ''): v 
                    for k, v in checkpoint["text_model_state"].items()}
        checkpoint["vision_adapter_state"] = {k.replace('base_model.model.', ''): v 
